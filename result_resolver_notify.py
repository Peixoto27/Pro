...
from config import HISTORY_FILE, SIGNALS_FILE, SYMBOLS, SEND_STATUS_UPDATES
...
def _utc_from_ts(ts):
    try: return datetime.utcfromtimestamp(int(ts))
...
def main():
    print("🧮 Resolver+Notify: carregando…")
    ...
    for rec in history:
        if rec.get("result") in ("hit_tp","hit_sl","expirado"):
            continue
        if rec.get("symbol"):
            pending_syms.add(rec["symbol"])

    if not pending_syms:
        print("ℹ️ Nada pendente."); return

    print(f"💱 Buscando preços de {len(pending_syms)} símbolos…")
    prices = fetch_bulk_prices(list(pending_syms))
    now = _now_utc(); updated = 0
    ...
    for rec in history:
        ...
        entry = rec.get("entry"); tp = rec.get("tp"); sl = rec.get("sl")
        ...
        if not pinfo or "usd" not in pinfo: continue
        cur = float(pinfo["usd"])
    ...
    if updated:
        ...
        print(f"💾 Atualizados {updated} registros no {HISTORY_FILE}.")
    else:
        print("ℹ️ Nenhuma atualização agora.")
